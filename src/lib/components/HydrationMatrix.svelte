<script lang="ts">
  ...
  const PROFILE_KEY = 'hydration_profile_data'; // Separate key for permanent profile data
  ...
  async function saveData() {
    ...
    try {
      // Save both to Firebase with different keys
      const dailySuccess = await FirebaseStorage.saveHydrationData(dailyData);
      const profileSuccess = await FirebaseStorage.saveHydrationData(profileData);
  ...
  function resetDailyProgress() {
    // Only reset daily consumption data, preserve user profile
    waterBottles = [0, 0, 0, 0];
    totalConsumed = 0;
