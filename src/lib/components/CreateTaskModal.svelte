...
{#if showModal}
  ...
  <div class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-end justify-center" on:click={closeModal}>
    <!-- Main Modal -->
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <div class="w-full max-w-md bg-gray-900/95 backdrop-blur-xl rounded-t-[20px] transform transition-all duration-300 ease-out" on:click|stopPropagation>
      ...
      <div class="py-2">
        {#each priorities as prio}
          <button
            ...
            on:click={() => { priority = prio.value; showPriorityDropdown = false; }}
          >
